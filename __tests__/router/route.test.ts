...
describe("Route", () => {
  ...
  it("should throw an error if the method or controller is not provided", () => {
    expect(() => route.addController(HttpMethods.GET, null as unknown as Controller)).toThrow(
      "Invalid parameters: method and handler are required."
    );
  });
  ...
  it("should throw an error if a controller is added for an already registered HTTP method", () => {
    route.addController(HttpMethods.GET, mockController);
    expect(() => route.addController(HttpMethods.GET, mockController)).toThrow(
      "Controller already registered for this HTTP method: GET"
    );
  });
  ...
  it("should throw an error for an unsupported HTTP method", () => {
    expect(() => route.addController("INVALID" as HttpMethods, mockController)).toThrow(
